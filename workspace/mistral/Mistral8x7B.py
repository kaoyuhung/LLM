...
def getModelandTokenizeer(mistral_models_path, node_id, gpu, group):
...
def my_main(
    ...
    prompts = load_data(benchmark)
    gpu = torch.device(f"cuda:{LOCAL_RANK}")
    dist.init_process_group(
        "nccl", rank=WORLD_RANK, world_size=WORLD_SIZE, device_id=gpu
    )
    group = dist.new_group(list(range(WORLD_SIZE)), use_local_synchronization=True)
    model, tokenizer = getModelandTokenizeer(model_path, node_id, gpu, group)
    model.eval()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model-path", type=str, default="mistral_weights/Mixtral-8x7B-Instruct-v0.1"
    )
