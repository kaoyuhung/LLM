...
output_file="measure_mistral8x7B_multinode.txt"
...
torchrun --nnodes=$nnodes --nproc_per_node=$nproc --node-rank=$node_rank --master-addr=$master_addr --master-port=$master_port --max-restarts 3 \
    run_mistral.py --node_rank $node_rank --model "Mistral-8x7B-Instruct-v0.1" --model_path "mistral_weights/Mistral-8x7B-Instruct-v0.1" --eval_nItrs $eval_nItrs --batch_size $batch_size \
    >> $output_folder/$output_file

# for N in 1 2 4 8 16
# do
#   torchrun --nnodes=$nnodes --nproc_per_node=$nproc --node-rank=$node_rank --master-addr=$master_addr --master-port=$master_port \
