...
export UCX_IB_GPU_DIRECT_RDMA=1 # allows direct memory access between the GPU and the network interface (e.g., Mellanox InfiniBand cards) without involving the CP
# enable NCCL log
...
if [ "$SLURM_JOB_NUM_NODES" -eq 1 ]; then
  ...
  SRUN_CMD="./run_multinode.sh $SLURM_JOB_NUM_NODES $SLURM_GPUS_PER_NODE $mode $eval_nItrs $model $model_path $output_folder" 
fi
...
echo "Task ID (MPI rank): $SLURM_PROCID"
